...
function seedTransactions() {
  ...
  allTransactions.forEach(t => {
    stmt.run(t.id, t.name, t.merchant, t.category, t.date, t.amount, t.icon, t.status);
  });
  stmt.finalize();
}
...
function seedCards() {
  ...
  mockCards.forEach(c => {
    stmt.run(c.id, c.bank, c.number, c.holder, c.expiry, c.type, c.gradient, c.network, c.isVirtual ? 1 : 0);
  });
  stmt.finalize();
}
